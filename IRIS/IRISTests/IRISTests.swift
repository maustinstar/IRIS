...
class IRISTests: XCTestCase {
    ...
    func attemptPrediction(for patch: Patch, with model: Model) {
    ...
    func testPredictionStarry() {
        ...
        // Create Patch
        let source = UIImage(named: "Sample.jpg", in: Bundle(for: type(of: self)), compatibleWith: nil)!
        let rect = CGRect(origin: CGPoint.zero, size: CGSize(width: 700, height: 720))
        let patch = source.cgImage!.patch(in: rect, at: (0 ,0))
        ...
        measure {
            do {
                ...
                _ = Patch(buffer: prediction.outputImage, position: (0, 0), size: (720, 720)).uiImage
            } catch {
                print("fail")
            }
        }
